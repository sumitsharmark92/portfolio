...
(function () {
  ...
  // ========== HIDDEN TERMINAL EASTER EGG ("sudo" key sequence) ==========
  let keySequence = '';
  ...
  function openSecretTerminal() {
    if (document.getElementById('secretTerminalModal')) return;

    const modal = document.createElement('div');
    modal.id = 'secretTerminalModal';
    modal.style.cssText = `
    ...
    modal.innerHTML = `
      <div style="width: 100%; max-width: 750px; height: 480px; background: #0c1017; border: 1px solid var(--green); border-radius: 8px; box-shadow: 0 0 40px rgba(0,255,65,0.3); display: flex; flex-direction: column; overflow: hidden; font-family: var(--font-mono);">
        <div style="padding: 10px 16px; background: #161b22; border-bottom: 1px solid rgba(0,255,65,0.2); display: flex; justify-content: space-between; align-items: center;">
          <span style="color: var(--green); font-size: 0.85rem; font-weight: bold;">root@sumit.sh: ~ (SECRET TERMINAL)</span>
        ...
        <div style="padding: 10px 16px; background: #161b22; border-top: 1px solid rgba(0,255,65,0.2); display: flex; align-items: center; gap: 8px;">
          <span style="color: var(--green);">root#</span>
          <input id="secretTermInput" type="text" style="flex: 1; background: none; border: none; outline: none; color: #fff; font-family: var(--font-mono); font-size: 0.85rem;" autofocus autocomplete="off">
        </div>
      </div>
    `;
    ...
    const input = document.getElementById('secretTermInput');
    const body = document.getElementById('secretTermBody');
    const closeBtn = document.getElementById('closeSecretTerm');
    ...
    input.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        ...
        line.innerHTML = `<span style="color: var(--green);">root#</span> ${escapeHtml(cmd)}`;
        body.appendChild(line);

        execSecretCmd(cmd.toLowerCase(), body, modal);
        body.scrollTop = body.scrollHeight;
      }
    });
  }

  function execSecretCmd(cmd, body, modal) {
    const out = document.createElement('div');
    out.style.marginBottom = '0.5rem';
    ...
    if (cmd === 'help') {
      ...
      out.innerHTML = `<span style="color:var(--red);">Command not recognized: ${escapeHtml(cmd)}. Type 'help' for options.</span>`;
    }
  ...
  console.log(
    '%c> Tip: Type "sudo" anywhere to unlock the secret terminal shell.',
